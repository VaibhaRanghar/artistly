  ...
  return (
    <DashboardLayout role="ARTIST">
      <div className="space-y-6">
        <div>
          <p className="text-[#f5e642] font-mono text-xs uppercase tracking-widest mb-1">// bookings</p>
          <h1 className="text-3xl font-black text-white">MY ORDERS & PROPOSALS</h1>
          <p className="text-white/40 mt-1">Track incoming bookings and your submitted event proposals.</p>
        </div>

        <Card className="bg-[#111] border-white/10">
          <CardHeader>
            <CardTitle>Active Orders</CardTitle>
            <CardDescription className="text-white/40">Manage your workload and order statuses.</CardDescription>
          </CardHeader>
          <CardContent>
            {orders.length === 0 ? (
              ...
              <div className="space-y-4">
                {orders.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="p-5 border-2 border-white/10 bg-black/40 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 group hover:border-white/30 transition-colors">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <Badge variant="outline" className={
                            order.status === "COMPLETED" ? "text-[#00ffcc] border-[#00ffcc]/30 bg-[#00ffcc]/10" : 
                            order.status === "PENDING" ? "text-[#f5e642] border-[#f5e642]/30 bg-[#f5e642]/10" : 
                            order.status === "IN_PROGRESS" ? "text-[#ff2a6d] border-[#ff2a6d]/30 bg-[#ff2a6d]/10" :
                            order.status === "CANCELLED" ? "text-red-500 border-red-500/30 bg-red-500/10" :
                      ...
                      <div className="w-full md:w-auto shrink-0 flex flex-col items-end gap-2">
                         <OrderActions orderId={order.id} status={order.status} isEventApplication={isEventApplication} />
                      </div>
                    </div>
                  );
                })}
